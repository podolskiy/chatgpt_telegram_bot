    ...
    async def message_handle_fn():
        ...
        try:
            ...
            db.append_dialog_message(
                user_id, new_dialog_message, dialog_id=None)
        ...
        if n_first_dialog_messages_removed > 0:
            if n_first_dialog_messages_removed == 1:
                ...
                text = f"✍️ <i>Note:</i> Your current dialog is too long, so <b>{
                    n_first_dialog_messages_removed} first messages</b> were removed from the context.\n Send /new command to start new dialog"
            await update.message.reply_text(text, parse_mode=ParseMode.HTML)
...
def get_chat_mode_menu(page_index: int):
    ...
    text = f"Select <b>chat mode</b> ({len(config.chat_modes)
                                       } modes available):"
...
async def show_balance_handle(update: Update, context: CallbackContext):
    ...
    for model_key in sorted(n_used_tokens_dict.keys()):
        ...
        details_text += f"- {model_key}: <b>{n_input_spent_dollars +
                                             n_output_spent_dollars:.03f}$</b> / <b>{n_input_tokens + n_output_tokens} tokens</b>\n"

    # image generation
    image_generation_n_spent_dollars = config.models["info"][
        "dalle-2"]["price_per_1_image"] * n_generated_images
    if n_generated_images != 0:
        details_text += f"- DALL·E 2 (image generation): <b>{
            image_generation_n_spent_dollars:.03f}$</b> / <b>{n_generated_images} generated images</b>\n"
    ...
    if n_transcribed_seconds != 0:
        details_text += f"- Whisper (voice recognition): <b>{
            voice_recognition_n_spent_dollars:.03f}$</b> / <b>{n_transcribed_seconds:.01f} seconds</b>\n"
...
async def error_handle(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        message = (
            ...
            f"<pre>update = {html.escape(json.dumps(
